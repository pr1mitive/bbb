...
(() => {
  ...
  async function calculateReceivedQty(poNumber, itemCode) {
    try {
      ...
      // ドロップダウンフィールドは in 演算子を使用
      const query = `${FIELDS.TRANSACTION.PO_NUMBER} = "${poNumber}" and ${FIELDS.TRANSACTION.ITEM_CODE} = "${itemCode}" and ${FIELDS.TRANSACTION.TRANSACTION_TYPE} in ("入庫") and ${FIELDS.TRANSACTION.STATUS} in ("確定")`;
