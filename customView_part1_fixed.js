...
(function() {
  ...
  kintone.events.on('app.record.index.show', async function(event) {
    ...
    try {
      ...
      // 初期明細行を1行追加
      if (typeof window.addItemRow === 'function') { addItemRow(); } else { console.error('[PO] addItemRow not defined at line 52'); setTimeout(() => { if (typeof window.addItemRow === 'function') addItemRow(); }, 500); }
  ...
  function attachEventListeners() {
    ...
    // 見積参照ボタン
    document.getElementById('btnQuoteRef').addEventListener('click', openQuoteModal);
    
    // 見積一括取込ボタン
    document.getElementById('btnBulkImport').addEventListener('click', openQuoteModal);
